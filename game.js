...
function drawPlayer() {
    ...
    // Determine eye expression based on movement
    const eyeSize = 5 + Math.abs(player.dx) / 4; // Eyes widen during fast movement
    const eyeY = player.height * (player.dy < 0 ? 0.25 : 0.35); // Eyes move up when jumping
    
    // Add eyes with pupils that follow movement direction
    const leftEyeX = player.width * 0.3;
    const rightEyeX = player.width * 0.7;
    ...
    // Add smile that changes with movement
    const smileY = player.height * 0.55;
    const smileWidth = 15 + player.dx * 0.5; // Smile widens with speed
    ctx.beginPath();
    
    if (player.dy < -2) {
        // Surprised mouth when jumping upward fast
        ctx.arc(player.width * 0.5, smileY, 7, 0, Math.PI * 2);
    } else if (player.dy > 2) {
        // Worried expression when falling
        ctx.arc(player.width * 0.5, smileY + 8, 10, Math.PI, Math.PI * 2);
    } else {
        // Normal smile, wider when moving fast
        ctx.arc(player.width * 0.5, smileY, smileWidth, 0, Math.PI);
    }
